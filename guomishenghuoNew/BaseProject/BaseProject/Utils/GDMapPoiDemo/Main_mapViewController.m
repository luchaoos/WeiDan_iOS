...
@interface Main_mapViewController () <MAMapViewDelegate,MapPoiTableViewDelegate,AMapSearchDelegate,UISearchBarDelegate,SearchResultTableVCDelegate>

@end

@implementation Main_mapViewController
{
    MAMapView *_mapView;
    // 地图中心点的标记
...
//    [self.navigationController pushViewController:locationDetailVC animated:YES];
    ...
    [dict setObject:[NSString stringWithFormat:@"%f",_tableView.selectedPoi.location.latitude] forKey:@"lat"];
    [dict setObject:[NSString stringWithFormat:@"%f",_tableView.selectedPoi.location.longitude] forKey:@"lon"];
    [dict setObject:[NSString stringWithFormat:@"%@",_tableView.selectedPoi.name] forKey:@"name"];
    [dict setObject:[NSString stringWithFormat:@"%@",_tableView.selectedPoi.address] forKey:@"address"];
    [[NSNotificationCenter defaultCenter]postNotificationName:@"select_address_compliat" object:dict];
