//
//  Index_GoodInfoViewController.m
//  BaseProject
//
//  Created by 于金祥 on 16/11/3.
...
- (void)viewDidLoad {
    ...
    NSURL * webUrl=[NSURL URLWithString:[NSString stringWithFormat:@"%@Product.aspx?productid=%@&buyerid=%@",BaseImgUrl,self.goodID,get_sp(user_ID)]];
...
-(void)webViewDidFinishLoad:(UIWebView *)webView
{
    @try {
        ...
        __weak Index_GoodInfoViewController *weakSelf = self;
        ...
        _jsContext[@"BuyRightNow"]=^(id productID,id num,id jifenRate)
        {
            [JSContext currentContext];
            jifenrate=[jifenRate floatValue];
            DataProviderOther *dataprovider=[[DataProviderOther alloc] init];
            [dataprovider setDelegateObject:weakSelf setSucceedBackFunctionName:@"BuyRightNowCallBack:" setFailBackFunctionName:nil];
            [dataprovider BuyRightNowWith:productID andnum:@"1"];
        };
