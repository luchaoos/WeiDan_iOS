...
- (void)viewDidLoad
{
    ...
    _topView.backgroundColor = NAVBAR_COLOR;
    ...
    _imgLeft.image = [UIImage imageNamed:@"fanhui"];
    ...
    _lblRight = [[UILabel alloc] initWithFrame:CGRectMake(SCREEN_WIDTH - 80 ,_orginY ,80,NavigationBar_HEIGHT)];
    ...
    self.tapGesture.enabled = NO;//默认关闭
...
{
    if (_tapGesture == nil) {
        _tapGesture = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(tapViewAction:) ];
...
//    self.floatButton.contentMode = UIViewContentModeScaleAspectFit;
    ...
    self.floatWindow.backgroundColor = ITEMS_COLOR;
