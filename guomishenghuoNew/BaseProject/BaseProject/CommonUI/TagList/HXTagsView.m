...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        _selectBackgroundColor=[self imageWithColor:[UIColor yellowColor] size:CGSizeMake(1.0, 1.0)];
    }
    return self;
}
...
- (void)setTagAry:(NSArray *)tagAry delegate:(id)delegate {
    ...
    for (NSArray *iTags in disposeAry) {
        ...
        for (NSDictionary *tagDic in iTags) {
            ...
            NSUInteger indexpath=[tagDic[@"indexpath"] intValue];
            ...
            [button setBackgroundImage:_selectBackgroundColor forState:UIControlStateSelected];
...
- (void)disposeTags:(NSArray *)ary {
    ...
    for (NSString *tagTitle in ary) {
        ...
        dict[@"indexpath"]=[NSString stringWithFormat:@"%d",i];
        if (index == 0) {
